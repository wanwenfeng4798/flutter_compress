...
class MethodChannelFlutterCompress extends FlutterCompressPlatform {
  final MethodChannel _method = const MethodChannel('flutter_compress_pro/methods');
  final EventChannel _progress = const EventChannel(
    'flutter_compress_pro/progress',
  );
  ...
  Stream<CompressionProgress> get progressStream {
    _progressStream ??= _progress.receiveBroadcastStream().map(
      (e) => CompressionProgress.fromMap((e as Map).cast<dynamic, dynamic>()),
    );
  ...
  ) async => CompressionEstimate.fromMap(
    await _invokeVideoMap('estimate', {'path': path, 'config': config.toMap()}),
  );
  ...
  Future<VideoCompressResult> compress(
    ...
    String? outputDir,
    String? outputName,
  ) async => VideoCompressResult.fromMap(
    await _invokeVideoMap('compress', {
      'id': id,
      'path': path,
      'config': config.toMap(),
      'outputDir': outputDir,
  ...
  Future<ImageCompressResult> compressImage(
    ...
    String? outputDir,
    String? outputName,
  ) async => ImageCompressResult.fromMap(
    await _invoke<Map<dynamic, dynamic>>(
      'compressImage',
      {
        'path': path,
  ...
  Future<ImageBytesResult> compressImageBytes(
    Uint8List source,
    ImageCompressConfig config,
  ) async => ImageBytesResult.fromMap(
    await _invoke<Map<dynamic, dynamic>>(
      'compressImageBytes',
